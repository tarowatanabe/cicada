...
namespace cicada
{
  ...
  private:
	...
	if (prune_bin)
	  edge.attributes.insert(std::make_pair(attr_prune_bin, attribute_set_type::int_type(item->in_edge->head)));
  ...
  inline
  void apply_cube_grow(const Model& model, const HyperGraph& source, HyperGraph& target, const Function& func, const int cube_size, const bool prune_bin=false)
  {
    ApplyCubeGrow<typename Function::value_type, Function>(model, func, cube_size, prune_bin)(source, target);
  }

  template <typename Function>
  inline
  void apply_cube_grow(const Model& model, HyperGraph& source, const Function& func, const int cube_size, const bool prune_bin=false)
  {
    HyperGraph target;
