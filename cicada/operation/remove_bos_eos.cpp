...
#include <cicada/remove_bos_eos.hpp>

#include <cicada/operation/remove_bos_eos.hpp>
...
namespace cicada
{
  namespace operation
  {
    RemoveBosEos::RemoveBosEos(const std::string& parameter, const int __debug)
      :  lattice_mode(false), forest_mode(false), debug(__debug)
    {
      typedef cicada::Parameter param_type;
	
      param_type param(parameter);
      if (utils::ipiece(param.name()) != "remove-bos-eos")
	throw std::runtime_error("this is not an BOS/EOS remover");
	...
	std::cerr << "WARNING: unsupported parameter for remove-bos-eos: " << piter->first << "=" << piter->second << std::endl;
      }
	...
	cicada::remove_bos_eos(data.lattice, removed);
	...
	cicada::remove_bos_eos(data.hypergraph, removed);
