...
#include <cicada/operation/push_head.hpp>
...
namespace cicada
{
  namespace operation
  {
    PushHead::PushHead(const std::string& parameter, const int __debug)
      :  base_type("push-head"), debug(__debug)
    {
      typedef cicada::Parameter param_type;
	
      param_type param(parameter);
      if (utils::ipiece(param.name()) != "push-head")
	throw std::runtime_error("this is not a head pusher");
	
      for (param_type::const_iterator piter = param.begin(); piter != param.end(); ++ piter)
	std::cerr << "WARNING: unsupported parameter for push-head: " << piter->first << "=" << piter->second << std::endl;
    }

    void PushHead::operator()(data_type& data) const
    {
	...
	std::cerr << name << ": " << data.id << std::endl;
      ...
      cicada::push_head(hypergraph, pushed);
