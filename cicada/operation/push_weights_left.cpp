...
#include <cicada/push_weights_left.hpp>

#include <cicada/operation/push_weights_left.hpp>
...
namespace cicada
{
  namespace operation
  {
    PushWeightsLeft::PushWeightsLeft(const std::string& parameter, const int __debug)
      :  base_type("push-weights-left"), debug(__debug)
    {
      typedef cicada::Parameter param_type;
	
      param_type param(parameter);
      if (utils::ipiece(param.name()) != "push-weights-left")
	throw std::runtime_error("this is not a weights-left pusher");
	
      for (param_type::const_iterator piter = param.begin(); piter != param.end(); ++ piter)
	std::cerr << "WARNING: unsupported parameter for push weights-left: " << piter->first << "=" << piter->second << std::endl;
    }

    void PushWeightsLeft::operator()(data_type& data) const
    {
