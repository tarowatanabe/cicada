...
namespace cicada
{
 ...
 //  title     = {Forest Rescoring: Faster Decoding with Integrated Language Models},
  ...
  inline
  void apply_cube_prune_rejection(const Model& model, const HyperGraph& source, HyperGraph& target, const Function& func, Sampler& sampler, const int cube_size)
  {
    ApplyCubePruneRejection<typename Function::value_type, Function, Sampler>(model, func, sampler, cube_size)(source, target);
  }

  template <typename Function, typename Sampler>
  inline
  void apply_cube_prune_rejection(const Model& model, HyperGraph& source, const Function& func, Sampler& sampler, const int cube_size)
  {
    HyperGraph target;
