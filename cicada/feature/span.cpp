...
namespace cicada
{
  namespace feature
  {
    ...
    public:
      ...
      {
	if (label_map.empty() || label_chart.empty()) return;
	...
	// try binary combination...
	for (int middle = span.first + 1; middle != span.second; ++ middle) {
	  const std::string& prev = label_chart(span.first, middle);
	  const std::string& next = label_chart(middle, span.second);
	...
	// try right-substitution...
	for (int last_super = span.second + 1; last_super < label_chart.size(); ++ last_super) {
	  const std::string& super = label_chart(span.first, last_super);
	  const std::string& right = label_chart(span.second, last_super);
	...
	// try left-subtraction...
	for (int first_super = span.first - 1; first_super >= 0; -- first_super) {
	  const std::string& super = label_chart(first_super, span.second);
	  const std::string& left  = label_chart(first_super, span.first);
