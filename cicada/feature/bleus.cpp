...
#include "cicada/feature/bleus.hpp"
#include "cicada/parameter.hpp"
#include "cicada/eval/bleus.hpp"
...
namespace cicada
{
  namespace feature
  {
	...
	if (skip_sgml_tag)
	  ...
	  return bleus_score(state, states, edge, final, skipper_epsilon());
      }
	...
	if (states.empty()) {
	  ...
	  *context_count = citer - const_cast<states_count_set_type&>(states_counts).begin();
	  ...
	  const double bleus = bleus_score(counts, *context_hypothesis, ! final);
	  
	  //std::cerr << "bleus: " << bleus << ' ';
	  ...
	  for (phrase_type::const_iterator titer = target.begin(); titer != titer_end; ++ titer) {
	    if (titer->is_non_terminal()) {
	      ...
	      bleus_antecedent += bleus_score(counts_antecedent, *antecedent_hypothesis, true);
	  ...
	  *context_count = citer - const_cast<states_count_set_type&>(states_counts).begin();
	  ...
	  const double bleus =  bleus_score(counts, *context_hypothesis, ! final) - bleus_antecedent;
	...
	if (hypothesis_size == 0 || reference_size == 0)
      ...
      double bleus_score(const count_set_type& __counts, const int hypothesis_size, const bool scaling=true) const
      {
	count_set_type counts_bleus(order, count_type(0));
	if (exact)
	  for (ngram_set_type::id_type id = 0; id < __counts.size();++ id)
	    counts_bleus[nodes[id].order - 1] += utils::bithack::min(int(__counts[id]), int(ngrams[id]));
    ...
    BleuS::BleuS(const std::string& parameter)
      : pimpl(0)
    {
      ...
      if (utils::ipiece(param.name()) != "bleus")
	throw std::runtime_error("this is not BleuS feature: " + parameter);
	...
	throw std::runtime_error("no refset file?: " + refset_file.string());
      ...
      std::unique_ptr<impl_type> bleus_impl(new impl_type(order, exact, skip_sgml_tag, tokenizer));
      
      // two-side context + length + counts-id 
      base_type::__state_size = sizeof(symbol_type) * order * 2 + sizeof(int) + sizeof(impl_type::id_type);
      base_type::__feature_name = (name.empty() ? std::string("bleus") : name);
      
      pimpl = bleus_impl.release();
    ...
    BleuS::~BleuS() { std::unique_ptr<impl_type> tmp(pimpl); }

    BleuS::BleuS(const BleuS& x)
      : base_type(static_cast<const base_type&>(x)),
	pimpl(new impl_type(*x.pimpl))
    {}

    BleuS& BleuS::operator=(const BleuS& x)
    {
