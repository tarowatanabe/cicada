...
#include "cicada/feature/ngram_rnn.hpp"
...
namespace cicada
{
  namespace feature
  {
    class NGramRNNImpl : public utils::hashmurmur3<size_t>
    {
    public:
      ...
      typedef cicada::NGramRNN      ngram_type;
    ...
    public:
      NGramRNNImpl(const path_type& __path, const int __order, const bool populate)
	...
	id_eos = ngram->vocab()[vocab_type::EOS];
      ...
      NGramRNNImpl(const NGramRNNImpl& x)
	...
	initialize_cache();
      ...
      NGramRNNImpl& operator=(const NGramRNNImpl& x)
      {
	ngram = &ngram_type::create(x.ngram->path());
	order = x.order;
    ...
    const NGramRNNImpl::symbol_type::id_type NGramRNNImpl::id_empty = NGramRNNImpl::symbol_type::id_type(-2);
    const NGramRNNImpl::symbol_type::id_type NGramRNNImpl::id_star  = NGramRNNImpl::symbol_type::id_type(-3);
    
    NGramRNN::NGramRNN(const std::string& parameter)
      : pimpl(0), pimpl_coarse(0)
    {
      ...
      if (utils::ipiece(param.name()) != "ngram-rnn")
	...
	throw std::runtime_error("no coarse ngram language model? " + coarse_path.string());
      ...
      base_type::__feature_name = (name.empty() ? std::string("ngram-rnn") : name);
    ...
    NGramRNN& NGramRNN::operator=(const NGramRNN& x)
    {
    ...
    void NGramRNN::apply(state_ptr_type& state,
			 const state_ptr_set_type& states,
			 const edge_type& edge,
    ...
    void NGramRNN::apply_coarse(state_ptr_type& state,
				const state_ptr_set_type& states,
				const edge_type& edge,
				feature_set_type& features,
				const bool final) const
    ...
    void NGramRNN::apply_predict(state_ptr_type& state,
				 const state_ptr_set_type& states,
				 const edge_type& edge,
				 feature_set_type& features,
				 const bool final) const
    ...
    void NGramRNN::apply_scan(state_ptr_type& state,
			      const state_ptr_set_type& states,
			      const edge_type& edge,
			      const int dot,
			      feature_set_type& features,
    ...
    void NGramRNN::apply_complete(state_ptr_type& state,
				  const state_ptr_set_type& states,
				  const edge_type& edge,
				  feature_set_type& features,
				  const bool final) const
