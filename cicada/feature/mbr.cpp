...
#include "cicada/feature/mbr.hpp"
...
namespace cicada
{
  namespace feature
  {
    ...
    public:
      MBRImpl(const int __order)
	: ngrams(symbol_type()), order(__order)
      {
	feature_names.resize(order);
	for (int n = 0; n < order; ++ n)
	  feature_names[n] = "mbr" + boost::lexical_cast<std::string>(n + 1);
      }
    ...
    MBR::MBR(const std::string& parameter)
      : pimpl(0)
    {
      ...
      if (param.name() != "mbr")
	throw std::runtime_error("is this mbr feature?" + parameter);
	...
	if (strcasecmp(piter->first.c_str(), "order") == 0)
	  ...
	  std::cerr << "WARNING: unsupported parameter for mbr: " << piter->first << "=" << piter->second << std::endl;
      }
	...
	throw std::runtime_error("invalid ngram order");
      ...
      base_type::__feature_name = "mbr";
      
      pimpl = new impl_type(order);
    }
    
    MBR::~MBR() { std::auto_ptr<impl_type> tmp(pimpl); }
    ...
    void MBR::apply(state_ptr_type& state,
			    const state_ptr_set_type& states,
			    const edge_type& edge,
    ...
    void MBR::apply_coarse(state_ptr_type& state,
				   const state_ptr_set_type& states,
				   const edge_type& edge,
				   feature_set_type& features,
				   feature_set_type& estimates,
    ...
    struct mbr_function
    {
      ...
      typedef MBR::weight_set_type weight_set_type;

      const weight_set_type& weights;

      mbr_function(const weight_set_type& __weights)
	: weights(__weights) {}
    ...
    void MBR::insert(const hypergraph_type& graph, const weight_set_type& weights)
    {
      typedef mbr_function::weight_type weight_type;
	...
	return;
      ...
      cicada::expected_ngram(graph, mbr_function(weights), ngrams, order, true);
