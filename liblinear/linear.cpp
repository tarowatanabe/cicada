...
static double solve_l2r_l1l2_svr(
...
static double train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	switch(param->solver_type)
	{
		...
		case L1R_L2LOSS_SVC:
		{
			...
			objective = solve_l1r_l2_svc(&prob_col, w, primal_solver_tol, Cp, Cn);
			delete [] prob_col.y;
			delete [] prob_col.x;
			delete [] x_space;
			break;
		...
		case L1R_LR:
		{
			...
			feature_node *x_space = NULL;
			transpose(prob, &x_space ,&prob_col);
			objective = solve_l1r_lr(&prob_col, w, primal_solver_tol, Cp, Cn);
			delete [] prob_col.y;
			delete [] prob_col.x;
			delete [] x_space;
			break;
		...
		case L2R_L1LOSS_SVR_DUAL:
			objective = solve_l2r_l1l2_svr(prob, w, param, L2R_L1LOSS_SVR_DUAL);
			break;
		case L2R_L2LOSS_SVR_DUAL:
			objective = solve_l2r_l1l2_svr(prob, w, param, L2R_L2LOSS_SVR_DUAL);
			break;
		default:
...
model* train(const problem *prob, const parameter *param)
{
	   ...
	   param->solver_type == L2R_L2LOSS_SVR_DUAL)
	{
		...
		model_->nr_class = 2;
		model_->label = NULL;
		model_->objective = train_one(prob, param, &model_->w[0], 0, 0);
	}
	else
	{
		int nr_class;
		...
		else
		{
			...
			else
			{
				...
				for(i=0;i<nr_class;i++)
				{
					...
					model_->objective += train_one(&sub_prob, param, w, weighted_C[i], param->C) / nr_class;
