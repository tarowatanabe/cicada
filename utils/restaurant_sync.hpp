// -*- mode: c++ -*-
//
//  Copyright(C) 2012-13 Taro Watanabe <taro.watanabe@nict.go.jp>
//


#ifndef __UTILS__RESTAURANT_SYNC__HPP__
#define __UTILS__RESTAURANT_SYNC__HPP__ 1
...
namespace utils
{
  ...
  public:
    ...
    restaurant_sync(const parameter_type& __parameter)
      : tables(),
	customers(),
    ...
    restaurant_sync(const double& __discount,
    ...
    restaurant_sync(const double& __discount_alpha,
  ...
  private:
    struct Location
    {
      ...
      Location() : counts(), mutex() {}
      Location(const Location& x) : counts(x.counts), mutex() {}
  ...
  public:
	...
	++ erased_customers;
	erased_tables += loc.counts.decrement(sampler).second;
      }
	...
	typename location_type::mutex_type::scoped_reader_lock lock(const_cast<location_type&>(dishes[dish]).mutex);
  ...
  private:    
    ...
    {
      DiscountSampler(const restaurant_sync& __crp) : crp(__crp) {}
      
      const restaurant_sync& crp;
    ...
    {
      StrengthSampler(const restaurant_sync& __crp) : crp(__crp) {}
...
namespace std
{
  template <typename A>
  inline
  void swap(utils::restaurant_sync<A>& x, utils::restaurant_sync<A>& y)
  {
    x.swap(y);
  }
