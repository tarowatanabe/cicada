...
#ifndef __UTILS__SYMBOL_SEQUENCE__HPP__
#define __UTILS__SYMBOL_SEQUENCE__HPP__ 1
...
//
// In addition to symbol, we assume
// hash(first, last)
// equal(value_type, first, last)
...
namespace utils
{
  ...
  struct __symbol_sequence_vector_impl {};
  
  template <typename Tp, typename Alloc>
  struct __symbol_sequence_vector_impl<Tp, Alloc, true>
  {
    typedef std::vector<Tp, Alloc> vector_type;
  };
  
  template <typename Tp, typename Alloc>
  struct __symbol_sequence_vector_impl<Tp, Alloc, false>
  {
    typedef utils::chunk_vector<Tp, 4096/sizeof(Tp), Alloc> vector_type;
  };

  template <typename Tp, bool Trivial>
  struct __symbol_sequence_clear {};
  
  template <typename Tp>
  struct __symbol_sequence_clear<Tp, true>
  {
    static inline
    void clear(Tp& value) {}
  };
  ...
  template <typename Tp>
  struct __symbol_sequence_clear<Tp, false>
  {
    static inline
    void clear(Tp& value)
    {
  ...
  class __symbol_sequence_base : public Hash,
				 public Equal
  {
  public:
  ...
  private:
    ...
    typedef __symbol_sequence_vector_impl<data_type, data_allocator_type, boost::has_trivial_copy<data_type>::value > impl_type;
    
    typedef typename impl_type::vector_type               data_set_type;
    typedef std::vector<index_type, index_allocator_type> bin_set_type;
    
  public:
    __symbol_sequence_base() : bins(8, 0), garbage(0) {}
    ...
    index_type insert(Iterator first, Iterator last)
    {
      const size_t key = hash()(first, last);
      index_type index = bins[key & (bins.size() - 1)];
      index_type index_empty = 0;
      
      for (/**/; index && ! equal()(data[index - 1].value, first, last); index = data[index - 1].next) {
	...
	data[index_empty - 1].value = value_type(first, last);
	...
	if (garbage) {
	  ...
	  data[index - 1].value = value_type(first, last);
	  ...
	  data.push_back(data_type(value_type(first, last), bins[pos], 1));
	...
	if (iter->counter) {
	  ...
	  __symbol_sequence_clear<value_type, boost::has_trivial_destructor<value_type>::value >::clear(iter->value);
	  iter->next = garbage;
	  garbage = pos + 1;
	}
      }
  ...
  template <typename Tp, typename Hash=boost::hash<Tp>, typename Equal=std::equal_to<Tp>, typename Alloc=std::allocator<Tp> >
  class symbol_sequence
  {
  public:
    typedef uint32_t id_type;
    
  public:
    symbol_sequence() : m_id(id_type(-1)) { allocate(Tp()); }
    symbol_sequence(const Tp& x) : m_id(id_type(-1)) { allocate(x); }
    template <typename Iterator>
    symbol_sequence(Iterator first, Iterator last) : m_id(id_type(-1)) { allocate(first, last); }
    symbol_sequence(const symbol_sequence& x) : m_id(x.m_id) { increment(); }
    ~symbol_sequence() { decrement(); m_id = id_type(-1); }
    
    symbol_sequence& operator=(const symbol_sequence& x)
    {
      assign(x);
      return *this;
    }
    ...
    void assign(const symbol_sequence& x)
    {
      if (x.m_id == m_id) return;
    ...
    friend
    std::size_t hash_value(symbol_sequence const& x) { return utils::hashmurmur<size_t>()(x.m_id, size_t(0)); }
    
    friend
    bool operator<(const symbol_sequence& x, const symbol_sequence& y) { return x.m_id < y.m_id; }
    friend
    bool operator<=(const symbol_sequence& x, const symbol_sequence& y) { return x.m_id <= y.m_id; }
    friend
    bool operator>(const symbol_sequence& x, const symbol_sequence& y) { return x.m_id > y.m_id; }
  ...
  private:
    typedef __symbol_sequence_base<Tp, Hash, Equal, Alloc> base_type;
    ...
    template <typename Iterator>
    void allocate(Iterator first, Iterator last)
    {
      m_id = base().insert(first, last);
    }
...
namespace std
{
  ...
  inline
  void swap(utils::symbol_sequence<T,H,E,A>& x, utils::symbol_sequence<T,H,E,A>& y)
  {
    x.swap(y);
  }
};
