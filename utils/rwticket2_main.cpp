...
#include "rwticket2.hpp"



struct writer_type
{
  writer_type(utils::rwticket2& __mutex,
	      int& __value) : mutex(__mutex), value(__value) {}
  
  void operator()()
  {
    for (int i = 0; i != 1024; ++ i) {
      utils::rwticket2::scoped_writer_lock lock(mutex);
  ...
  utils::rwticket2& mutex;
  int& value;
};

struct reader_type
{
  reader_type(utils::rwticket2& __mutex,
	      int& __value) : mutex(__mutex), value(__value) {}
  ...
  void operator()()
  {
    size_t accessed = 0;
    while (value != 1024) {
      utils::rwticket2::scoped_reader_lock lock(mutex);
