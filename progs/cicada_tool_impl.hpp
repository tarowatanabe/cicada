...
void process(const operation_set_type& operations)
{
  ...
  for (operation_set_type::const_iterator iter = operations.begin(); iter != operations.end(); ++ iter) {
	...
	os << *id_graph.second << std::endl;
      ...
      const bool source = (param.find("source") != param.end() ? true_false(param.find("source")->second) : false);
	...
	if (path_weights.empty()) {
	  if (unique)
	    kbest_derivations(os, id_graph.first, *id_graph.second, kbest_size, kbest_source_traversal(), kbest_function_one(weights), kbest_filter_unique(*id_graph.second));
	  else
	    kbest_derivations(os, id_graph.first, *id_graph.second, kbest_size, kbest_source_traversal(), kbest_function_one(weights), kbest_filter(*id_graph.second));
	} else {
	  utils::compress_istream is(path_weights);
	  is >> weights;
	  
	  if (unique)
	    kbest_derivations(os, id_graph.first, *id_graph.second, kbest_size, kbest_source_traversal(), kbest_function(weights), kbest_filter_unique(*id_graph.second));
	  else
	    kbest_derivations(os, id_graph.first, *id_graph.second, kbest_size, kbest_source_traversal(), kbest_function(weights), kbest_filter(*id_graph.second));
	}
      } else {
	if (path_weights.empty()) {
	  if (unique)
	...
	throw std::runtime_error("no graph?");
      ...
      stack.pop_back();
    } else if (param.name() == "binarize") {
      const int binarize_size = (param.find("size") != param.end() ? boost::lexical_cast<int>(param.find("size")->second) : 0);
	
      if (stack.empty())
	throw std::runtime_error("no graph?");
	
      binarize(*stack.back().second, binarize_size);

    } else if (param.name() == "permute") {
      const int permute_size = (param.find("size") != param.end() ? boost::lexical_cast<int>(param.find("size")->second) : -1);
	
      if (stack.empty())
	throw std::runtime_error("no graph?");
	
      permute(*stack.back().second, permute_size);
    } else
