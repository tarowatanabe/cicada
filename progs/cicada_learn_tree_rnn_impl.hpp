...
#ifndef __CICADA_LEARN_TREE_RNN__IMPL__HPP__
#define __CICADA_LEARN_TREE_RNN__IMPL__HPP__ 1
...
struct LearnBase
{
  ...
  struct State
  {
    State() : terminal_(vocab_type::EMPTY), node_(size_type(-1)), final_(false) {}
    
    bool final() const { return final_; }
    bool initial() const { return terminal_ == vocab_type::EMPTY && node_ == size_type(-1); }
    bool terminal() const { return ! initial() && terminal_ != vocab_type::EMPTY; }
	       ...
	       const tree_rnn_type& theta)
  {
	...
	if (! no_bos_eos_ && final) {
	  words_.clear();
	  words_.push_back(vocab_type::BOS);
	  words_.insert(words_.end(), edge.rule->rhs.begin(), edge.rule->rhs.end());
	  words_.push_back(vocab_type::EOS);
	  
	  if (skip_sgml_tag_)
	    forward(edge, words_.begin(), words_.end(), weights, W, theta, skipper_sgml());
	  else
	    forward(edge, words_.begin(), words_.end(), weights, W, theta, skipper_epsilon());
	} else {
	  if (skip_sgml_tag_)
	       ...
	       Skipper skipper)
  {
	...
	state_next.layer_ = (theta.Bt_
			     ...
			     + theta.Wt_.block(0, offset2, theta.hidden_, theta.embedding_) * theta.input_->operator()(*first)
			     ).array().unaryExpr(tree_rnn_type::shtanh());
	state_next.delta_ = tensor_type::Zero(theta.hidden_, 1);
	state_next.terminal_ = *first;
      }
    }
		...
		gradient_type& gradient)
  {
	...
	if (state.terminal()) { // terminal
	  ...
	  gradient.theta_.Wt_.block(0, offset2, theta.hidden_, theta.embedding_)
	    += state.delta_ * theta.input_->operator()(state.terminal_).transpose();
...
struct ViolationRoot : public ViolationBase
{
...
struct ViolationFrontier : public ViolationBase
{
  typedef cicada::semiring::Tropical<double> weight_type;
