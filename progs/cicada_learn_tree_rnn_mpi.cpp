...
int main(int argc, char ** argv)
{
  ...
  try {
    ...
    if (int(mix_none_mode) + mix_average_mode + mix_select_mode)
    ...
    for (model_type::const_iterator miter = model.begin(); miter != miter_end; ++ miter) {
      if (tree_rnn_feature && dynamic_cast<const cicada::feature::TreeRNN*>(miter->get()))
	throw std::runtime_error("We do not allow multiple tree-rnn features!");
      
      tree_rnn_feature = dynamic_cast<const cicada::feature::TreeRNN*>(miter->get());
    }
  ...
  for (int iter = 0; iter != iteration; ++ iter) {
    ...
    if (dump_weights_mode && mpi_rank == 0)
      queue_dumper.push(model_file_type(weights,
					theta,
					add_suffix(output_file, "." + utils::lexical_cast<std::string>(iter + 1)),
