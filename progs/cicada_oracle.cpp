  ...
  void operator()()
  {
	...
	*score_curr -= *scores[id];
	...
	__bleu_linear->assign(score_curr);
      ...
      sentences[id].clear();
      cicada::viterbi(graph_oracle, sentences[id], weight, kbest_traversal(), bleu_function(feature_bleu, score_factor));
      
      score_ptr_type score_sample = scorers[id]->score(sentences[id]);
      if (score_curr)
