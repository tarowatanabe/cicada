...
int main(int argc, char** argv)
{
  ...
  try {
	...
	learn_online(LearnAdaDelta(dimension_embedding, dimension_hidden, span, window, lambda, eta0),
		     bitexts,
		     dict_source_target,
		     dict_target_source,
		     theta);
...
struct TaskAccumulate
{
  ...
  TaskAccumulate(const Learner& learner,
    ...
    : learner_(learner),
      ...
      gradient_(theta.embedding_, theta.hidden_, theta.span_, theta.window_),
      gradient_batch_(theta.embedding_, theta.hidden_, theta.span_, theta.window_),
