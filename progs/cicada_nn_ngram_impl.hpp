...
private:
  ...
  void read(std::istream& is, tensor_type& embedding, const bool bias_last)
  {
    ...
    if (cols > embedding.cols())
      embedding.conservativeResize(dimension_embedding_ + bias_last, cols);
	...
	embedding.conservativeResize(dimension_embedding_ + bias_last, word.id() + 1);
      ...
      is.read((char*) embedding.col(word.id()).data(), sizeof(tensor_type::Scalar) * (dimension_embedding_ + bias_last));
    }
  }
