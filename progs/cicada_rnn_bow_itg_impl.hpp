...
#ifndef __CICADA_RNN_BOW_ITG_IMPL__HPP__
#define __CICADA_RNN_BOW_ITG_IMPL__HPP__ 1
...
struct Model
{
		  ...
		  Gen& gen)
  {
    ...
    const double range_o = std::sqrt(6.0 / (hidden_ + 1));
    ...
    output_source_ = tensor_type::Zero(hidden_ + 1, vocabulary_size).array().unaryExpr(randomize<Gen>(gen, range_o));
    output_target_ = tensor_type::Zero(hidden_ + 1, vocabulary_size).array().unaryExpr(randomize<Gen>(gen, range_o));
...
private:
  void write(std::ostream& os) const
  {
    ...
    write_embedding(os, output_source_, hidden_);
    write_embedding(os, output_target_, hidden_);
  }
  ...
  void read(std::istream& is)
  {
    ...
    read_embedding(is, output_source_, hidden_);
    read_embedding(is, output_target_, hidden_);
...
struct ITG
{
  ...
  template <typename Output, typename Hidden>
  double score_span(const word_type& word,
		    const Eigen::MatrixBase<Output>& output,
		    const double& scale,
		    const size_type& hidden_size,
		    const Eigen::MatrixBase<Hidden>& layer)
  {
		    ...
		    const Eigen::MatrixBase<Hidden>& layer)
  {
    ...
    const difference_type source_first = span.source_.first_;
    const difference_type source_last  = span.source_.last_;
    const difference_type target_first = span.target_.first_;
    const difference_type target_last  = span.target_.last_;
    
    double score = 0.0;
    
    for (difference_type i = source_first - window_size; i != source_first; ++ i)
      score += score_span(i < 0 ? vocab_type::BOS : source[i], theta.output_source_, theta.scale_, theta.hidden_, layer);
    
    for (difference_type i = source_last; i != source_last + window_size; ++ i)
      score += score_span(i >= source_size ? vocab_type::EOS : source[i], theta.output_source_, theta.scale_, theta.hidden_, layer);
    
    for (difference_type i = target_first - window_size; i != target_first; ++ i)
      score += score_span(i < 0 ? vocab_type::BOS : target[i], theta.output_target_, theta.scale_, theta.hidden_, layer);
    
    for (difference_type i = target_last; i != target_last + window_size; ++ i)
      score += score_span(i >= target_size ? vocab_type::EOS : target[i], theta.output_target_, theta.scale_, theta.hidden_, layer);
	       ...
	       const model_type& theta)
  {
    ...
    state.score_  = score_span(source, target, span, theta, state.layer_);
    state.weight_ = 0;
	       ...
	       const bool straight)
  {
    ...
    state.score_  = state1->score_ + state2->score_ + score_span(source, target, span, theta, state.layer_);
    state.weight_ = 0;
  ...
  template <typename Output, typename Hidden>
  void update_span(const word_type& word,
		   const Eigen::MatrixBase<Output>& output,
		   const double& scale,
		   const size_type& hidden_size,
		   const Eigen::MatrixBase<Hidden>& layer,
		   ...
		   tensor_type& delta)
  {
    grad.block(0, 0, hidden_size, 1)         += loss * layer;
    grad.block(hidden_size, 0, 1, 1).array() += loss;
    
    delta.array() += (layer.array().unaryExpr(dhinge())
		      * (output.col(word.id()).block(0, 0, hidden_size, 1) * loss * scale).array());
  }
  
  template <typename Hidden, typename Gen>
  double sample_span(const sentence_type& source,
		     const sentence_type& target,
		     const span_pair_type& span,
		     const model_type& theta,
		     const Eigen::MatrixBase<Hidden>& layer,
		     ...
		     Gen& gen)
  {
    ...
    for (difference_type i = source_first - window_size; i != source_first; ++ i) {
      ...
      update_span(word,
		  ...
		  theta.hidden_,
		  layer,
		  weight * (- label + sigmoid),
		  gradient.output_source(word), 
		  delta);
	...
	update_span(sampled,
		    ...
		    layer,
		    weight * (- label + sigmoid),
		    gradient.output_source(sampled),
		    delta);
      }
    }
	...
	update_span(sampled,
		    ...
		    layer,
		    weight * (- label + sigmoid),
		    gradient.output_target(sampled),
		    delta);
      }
    }
		  ...
		  Gen& gen)
  {
    ...
    for (state_set_type::const_iterator citer = citer_begin; citer != citer_end; ++ citer)
      logsum = utils::mathop::logsum(logsum, double((*citer)->score_));
	...
	// compute classificatin error...

	loss += sample_span(source, target, state.span_, theta, state.layer_, state.weight_, gradient, state.delta_, gen);
