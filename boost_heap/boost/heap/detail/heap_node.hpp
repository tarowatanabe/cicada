...
bool is_heap(const Node * n, typename HeapBase::compare_type const & cmp)
{
    for (typename Node::const_child_iterator it = n->children.begin(); it != n->children.end(); ++it)
    {
        Node const & this_node = const_cast<Node const &>(*it);
...
template <typename List, typename Node, typename Cmp, typename ValueExtractor>
Node * find_max_child(List const & list, Cmp const & cmp)
{
    ...
    for (typename List::const_iterator it = list.begin(); it != list.end(); ++it)
    {
        ...
        if (cmp(v(ret->value), v(current->value)))
            ret = current;
    }
